...
describe('RepositoriesComponent', () => {
  ...
  it('should render repository items when userRepos input is provided', () => {
    ...
    const repositoryItems = fixture.nativeElement.querySelectorAll('.bg-slate-100.border.shadow-lg.rounded-xl.p-4.text-xs.flex.flex-col.justify-between');
    ...
    expect(firstRepositoryItem.querySelector('p.text-slate-500').textContent).toBe('November 8, 2023');
    expect(firstRepositoryItem.querySelector('p.text-slate-700').textContent).toBe('This is a test repository.');
    expect(firstRepositoryItem.querySelector('a.text-green-500').textContent).toBe('test-repo-1.com');
    ...
    expect(languagesContainer.querySelectorAll('.bg-green-500.text-white').length).toBe(3);
    expect(languagesContainer.querySelector('.bg-green-500.text-white').textContent).toBe('JavaScript');
    expect(languagesContainer.querySelector('.bg-green-500.text-white + .bg-green-500.text-white').textContent).toBe('HTML');
    expect(languagesContainer.querySelector('.bg-green-500.text-white:last-child').textContent).toBe('CSS');
  });
